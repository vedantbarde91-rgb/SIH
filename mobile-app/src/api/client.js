...
//
// Uses VITE_API_BASE_URL from .env (your laptop's LAN IP for demo).
// Falls back to localhost for local browser dev.
//
// Set in .env:
    ...
    try {
      ...
      if (cached) {
        ...
        if (params && typeof params === 'object') {
          ...
          if (params.district && params.district !== 'ALL') list = list.filter((r) => (r.district || 'Dima Hasao').toLowerCase() === params.district.toLowerCase());
          if (params.user_id) list = list.filter((r) => r.user_id === params.user_id || r.phone_number === params.user_id);
